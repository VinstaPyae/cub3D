...
// Fixed isWall function - X should be column, Y should be row
int isWall(int x, int y, t_game *game)
{
    int col = x;  // X = column
    int row = y;  // Y = row
    
    if (col < 0 || col >= game->config->map_width || row < 0 || row >= game->config->map_height)
        return 1;
    return (game->config->map[row][col] == '1');  // Use row, col for array access
}
...
int raycast(t_game *game)
{
    ...
    while (x < SCN_WIDTH)
    {
        ...
        while (ray->hit == 0)
        {
            ...
            // Check bounds and walls - FIXED coordinate system
